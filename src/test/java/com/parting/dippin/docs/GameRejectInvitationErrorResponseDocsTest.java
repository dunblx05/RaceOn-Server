...
import com.parting.dippin.docs.error_response.GameRejectInvitationErrorCodeController;
 ...
 */
@DisplayName("[DocumentationTest] 게임 초대 거절 에러코드 테스트")
class GameRejectInvitationErrorResponseDocsTest extends RestDocsExceptionSupport {

    @Override
    protected Object initController() {
        return new GameRejectInvitationErrorCodeController();
    }

    @Test
    @DisplayName("게임 초대 거절에서 사용된 응답 코드를 조회하면 200 OK를 반환한다.")
    void errorCodeResponseTest() throws Exception {
        CodeAndMessage[] codeAndMessages
                = extractGameStartCodeAndMessage(GameCodeAndMessage.values(), GameRejectInvitationException.class);
        CodeAndMessage[] gameRejectInvitationCodeAndMessages = Arrays.copyOf(codeAndMessages, codeAndMessages.length + 1);
        gameRejectInvitationCodeAndMessages[gameRejectInvitationCodeAndMessages.length - 1] = CommonCodeAndMessage.BAD_REQUEST;

        FieldDescriptor[] descriptors = convertToFieldDescriptorArray(gameRejectInvitationCodeAndMessages);
        ...
        ResultActions result = this.mockMvc.perform(
                get("/api/specs/game-reject-error-response")
