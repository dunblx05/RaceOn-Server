...
@RestController
@RequestMapping("/api/specs/game-stop-error-response")
public class GameStopErrorCodeController {

    @GetMapping
    public ErrorCodeSpecs getErrorCodes() {
        List<CodeAndMessage> gameStopCodeAndMessages
                = new java.util.ArrayList<>(Arrays.stream(
                        extractGameStartCodeAndMessage(GameCodeAndMessage.values(), GameStopException.class))
                        .toList()
        );

        gameStopCodeAndMessages.add(CommonCodeAndMessage.BAD_REQUEST);

        Map<String, ErrorCodeSpec> errorCodes = new HashMap<>();

        for (CodeAndMessage codeAndMessage : gameStopCodeAndMessages) {
