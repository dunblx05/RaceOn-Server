...
@RestController
@RequestMapping("/api/specs/game-reject-error-response")
public class GameRejectInvitationErrorCodeController {

    @GetMapping
    public ErrorCodeSpecs getErrorCodes() {
        List<CodeAndMessage> gameRejectInvitationCodeAndMessages
        ...
        gameRejectInvitationCodeAndMessages.add(CommonCodeAndMessage.BAD_REQUEST);

        Map<String, ErrorCodeSpec> errorCodes = new HashMap<>();
        for (CodeAndMessage codeAndMessage : gameRejectInvitationCodeAndMessages) {
