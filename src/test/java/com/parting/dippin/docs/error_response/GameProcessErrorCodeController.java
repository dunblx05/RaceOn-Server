...
@RestController
@RequestMapping("/api/specs/game-process-error-response")
public class GameProcessErrorCodeController {

    @GetMapping
    public ErrorCodeSpecs getErrorCodes() {
        List<CodeAndMessage> gameProcessCodeAndMessages
                = new java.util.ArrayList<>(Arrays.stream(
                        extractGameStartCodeAndMessage(GameCodeAndMessage.values(), GameProcessException.class))
                        .toList()
        );

        gameProcessCodeAndMessages.add(CommonCodeAndMessage.BAD_REQUEST);

        Map<String, ErrorCodeSpec> errorCodes = new HashMap<>();

        for (CodeAndMessage codeAndMessage : gameProcessCodeAndMessages) {
