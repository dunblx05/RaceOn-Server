...
import com.parting.dippin.docs.error_response.GameStopErrorCodeController;
 ...
 */
@DisplayName("[DocumentationTest] 게임 중단 에러코드 테스트")
class GameStopErrorResponseDocsTest extends RestDocsExceptionSupport {

    @Override
    protected Object initController() {
        return new GameStopErrorCodeController();
    }

    @Test
    @DisplayName("게임 중단에서 사용된 응답 코드를 조회하면 200 OK를 반환한다.")
    void errorCodeResponseTest() throws Exception {
        CodeAndMessage[] codeAndMessages
                = extractGameStartCodeAndMessage(GameCodeAndMessage.values(), GameStopException.class);
        ...
        ResultActions result = this.mockMvc.perform(
                get("/api/specs/game-stop-error-response")
