...
import com.parting.dippin.docs.error_response.GameStartErrorCodeController;
 ...
 */
@DisplayName("[DocumentationTest] 게임 시작 요청 에러코드 테스트")
class GameStartErrorResponseDocsTest extends RestDocsExceptionSupport {

    @Override
    protected Object initController() {
        return new GameStartErrorCodeController();
    }

    @Test
    @DisplayName("게임 시작에서 사용된 응답 코드를 조회하면 200 OK를 반환한다.")
    void errorCodeResponseTest() throws Exception {
        ...
        CodeAndMessage[] gameStartCodeAndMessages = Arrays.copyOf(codeAndMessages, codeAndMessages.length + 1);
        gameStartCodeAndMessages[gameStartCodeAndMessages.length - 1] = CommonCodeAndMessage.BAD_REQUEST;

        FieldDescriptor[] descriptors = convertToFieldDescriptorArray(gameStartCodeAndMessages);
        ...
        ResultActions result = this.mockMvc.perform(
                get("/api/specs/game-start-error-response")
